...
- 优先复用现有样式和组件，不另换设计体系。房态到排场、按天到时段是业务适配；住房权益和账务不能仅改名继承。历史截图仅供风格参考，导航与 AI 能力以 v1.4.3 源码和当前规格核对。
...
- `.github/upstream-workflows` 与 `docs/upstream` 是停用的参考，网球生产环境尚未配置。禁止用上游发布工具部署网球代码。
