...
await page.getByLabel("姓名", { exact: true }).fill("草稿资料保留");
...
await expect(page.getByLabel("姓名", { exact: true })).toHaveValue("草稿资料保留");
...
await page.getByLabel("姓名", { exact: true }).fill("超时恢复验收客");
const recoveryPhone = reserveSyntheticPhone();
await page.getByLabel("手机号", { exact: true }).fill(recoveryPhone);
let lost = false;
...
await page.getByRole("button", { name: /查看办理结果/ }).waitFor();
await page.getByRole("button", { name: /查看办理结果/ }).click();
...
await m.getByLabel("姓名", { exact: true }).fill("手机体验临时客");
await m.getByLabel("手机号", { exact: true }).fill(reserveSyntheticPhone());
