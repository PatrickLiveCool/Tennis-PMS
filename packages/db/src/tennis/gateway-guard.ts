...
export function isGatewayActor(actor: TenantActor): boolean {
  return bindings.has(actor);
}
async function assertBinding(
  ...
  const row = await tx.query(
    ...
    WHERE b.tenant_id=$1 AND b.id=$2 AND b.subject_id=$3 AND b.active AND i.active AND t.active
      AND i.paused_at IS NULL AND (i.expires_at IS NULL OR i.expires_at>clock_timestamp())
