...
import { assertGatewayIdentity, assertGatewayDelegation } from "./gateway-guard.ts";
...
/** Metadata exists only for an actor resolved from a verified bearer token. */
export function getTrustedDelegationContext(actor: TenantActor): Readonly<Omit<Delegation, "tokenHash">> | null {
  const bound = bindings.get(actor);
  if (!bound) return null;
  const { tokenHash: _tokenHash, ...context } = bound;
...
export async function assertDelegation(tx: pg.PoolClient, actor: TenantActor, venueId?: string): Promise<void> {
  await assertGatewayIdentity(tx, actor);
  const bound = bindings.get(actor);
  if (!bound) return;
  ...
  await assertGatewayDelegation(tx, actor, bound.tokenHash);
}
