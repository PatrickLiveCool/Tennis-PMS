# Tennis-Green-PMS 生产发布与恢复 Runbook

> 本文是待接入草案。目标仓库 `PatrickLiveCool/Tennis-PMS` 的发布、回退和 retention workflow 仍停用；不得将下文的触发条件或权限约定视为已生效。现有 CI 完整保留，冲突差异见 [基础设施迁移记录](tennis-green-pms-infrastructure.md)。生产启动只读核对 Tennis 迁移校验和，不自动迁移；版本端点为 `/version`，同时校验版本和 Git revision。任何生产环境、云端权限、迁移和发布操作均须另外授权。
...
业务 PR 合并到 main
  ...
  -> release.published 触发 Tennis-Green-PMS Release
...
GitHub Release 的 **Publish release** 是批准动作。只有一个 `production` Environment，允许 `main` 和 `v*`，不配置 reviewer 或 wait timer，因此 workflow 不会再暂停等待第二次批准。release、rollback 和定时 retention 共用 `tennis-green-pms-production` concurrency，避免上传、切换和清理并行破坏状态。
...
Release Please 创建的 tag 必须严格匹配 `vX.Y.Z`，并指向 `main` 历史中的提交。Actions 同时核对 tag、package/lock、Release Please 生成的 `CHANGELOG` 条目和 `deploy/release-policy.json`。服务器不从 Git checkout 构建，生产 Compose 只接受明确的预构建 `TENNIS_GREEN_PMS_IMAGE`，固定容器名是 `tennis-green-pms-app`，Compose 项目名是 `tennis-green-pms`。

镜像使用不可变本地 tag：

```text
tennis-green-pms:vX.Y.Z-<40 位 Git revision>
```
...
```text
tennis-green-pms/releases/vX.Y.Z/<40 位 revision>/
  tennis-green-pms-linux-amd64.docker.tar.zst
  manifest.json
  SHA256SUMS
  sbom.spdx.json
  deployed.json
...
```text
qcs::cos:<region>:uid/<appid>:<bucket>/tennis-green-pms/releases/*
```

List 使用 bucket resource 时必须带 `tennis-green-pms%2Freleases%2F` 的 prefix condition。不要授予其他桶、数据库备份、TencentDB 或其他项目权限。COS 桶必须是私有、TLS、服务端加密，并且从未启用版本控制；`Enabled` 和 `Suspended` 都拒绝，因为 delete marker 和历史 object version 会破坏当前的不可变/删除假设。Lifecycle 只用于终止过期未完成分片上传，不能代替数量 retention。
...
本地镜像按 image ID 去重，而不是按 semantic、commit、`latest` 或 `pre-*` 标签计数。只处理 Tennis-Green-PMS 自己的标签；当前、previous 和任意容器引用都保护。不能删除的共享 image ID 保留并报告。绝不运行 `docker system prune` 或 `docker image prune -a`，绝不删除 volumes、TencentDB、数据库备份、其他项目镜像或容器。

## 5. 服务器切换和状态

服务器受限入口只接受已验证的版本、40 位 revision、固定 `tennis-green-pms/releases/` key 和 manifest SHA。SSH key 使用 `restrict`，固定 known_hosts，禁止 TTY、转发、任意 shell 和任意 Docker 命令。root-owned sudo wrapper 只允许 deploy、rollback 和 maintenance 固定入口；adopt、recover、`rollback-local` 和 `configure-ai` 只可由管理员本地执行。
...
4. 用不可变的版本/revision 镜像 tag 设置 `TENNIS_GREEN_PMS_IMAGE`，启动固定的 `tennis-green-pms` Compose 项目并切换 `tennis-green-pms-app`。
5. 等待 app Docker healthcheck，并检查本地和公网 `/health` 地址。
...
服务器只保留当前运行 image 和一个快速回退 image，具体按唯一 image ID 判断。任一受管容器仍引用旧 image 时清理会拒绝删除。生产 `.env`、COS reader credentials、state、audit 和数据库仍在 release 外部；生产服务器不保存源码构建目录、镜像 archive 或 BuildKit cache。

## 6. 回退

### GitHub Actions 回退

`.github/workflows/rollback.yml` 只在 `main` 上手工运行，输入 `version` 必填，`revision` 只在同一版本存在歧义时填写。`orchestrate.py rollback-release` 会列举 Tennis-Green-PMS release 前缀，并且只接受一份完整、checksum 正确、有有效 `deployed.json` 的成功版本；没有 marker 的候选、上传不完整的版本和损坏版本一律不能成为回退目标。
...
以下命令是管理员在明确授权后执行的一次性 bootstrap，不是日常发布命令。管理员在本地 Tennis-Green-PMS checkout 执行，并使用一个已有的管理员 SSH key。GitHub Actions 只使用一把专用 deploy key。

操作前将占位符替换为现场值：`$ADMIN_KEY` 是管理员私钥路径（例如 `$HOME/.ssh/<admin-key>`）；`$DEPLOY_KEY` 是唯一 deploy 私钥路径（例如 `$HOME/.ssh/tennis-green-pms-actions-deploy`）；`$ADMIN_ALIAS` 是管理员 SSH alias；`$REMOTE_STAGE` 是服务器临时目录；`$REMOTE_SETUP` 是服务器上的 root-owned 安装目录；`$LEGACY_CHECKOUT` 是服务器上旧 checkout 的绝对路径。以下 shell 片段假定这些变量已在管理员本地 shell 中设置。
...
rtk proxy ssh-keygen -t ed25519 -N '' \
  -C tennis-green-pms-actions-deploy \
  -f "$DEPLOY_KEY"
rtk git archive --format=tar.gz \
  --output=/tmp/tennis-green-pms-harness.tar.gz \
...
rtk proxy ssh "$ADMIN_ALIAS" 'cat /etc/ssh/ssh_host_ed25519_key.pub' \
  > /tmp/tennis-green-pms-host-key.pub
rtk proxy awk -v host=YOUR_SERVER_HOST \
  '{print host, $1, $2}' /tmp/tennis-green-pms-host-key.pub \
  > /tmp/tennis-green-pms-known_hosts
```

将 `/tmp/tennis-green-pms-known_hosts` 的整行内容填入 GitHub `DEPLOY_KNOWN_HOSTS`；它应是 `主机 ssh-ed25519 BASE64_PUBLIC_KEY`，不是只填 fingerprint。该文件不是私钥，但必须保留为单行并与 `DEPLOY_HOST` 相同。
...
rtk proxy scp /tmp/tennis-green-pms-harness.tar.gz \
  "$ADMIN_ALIAS:$REMOTE_STAGE/tennis-green-pms-harness.tar.gz"
rtk proxy scp "$DEPLOY_KEY.pub" \
  "$ADMIN_ALIAS:$REMOTE_STAGE/tennis-green-pms-deploy.pub"
rtk proxy ssh "$ADMIN_ALIAS" "sudo install -d -m 0700 '$REMOTE_SETUP'"
rtk proxy ssh "$ADMIN_ALIAS" "sudo tar -xzf '$REMOTE_STAGE/tennis-green-pms-harness.tar.gz' -C '$REMOTE_SETUP'"
rtk proxy ssh "$ADMIN_ALIAS" "sudo install -m 0644 '$REMOTE_STAGE/tennis-green-pms-deploy.pub' '$REMOTE_SETUP/deploy.pub'"
...
`deploy/install.sh` 创建 `tennis-green-pms-deploy`、root-owned forced-command key、固定 sudo wrapper、独立 Python venv、Compose 副本、recovery service/timer 和 logrotate 配置。它不启动应用、不执行 Docker build/load、不执行 adoption、不启用 timer、不修改数据库，也不修改或重载 sshd。安装器不能被部署用户自己改写；如果已有受管文件内容不同，会停止并要求管理员复核。

确认以下结果后再继续：

- `/home/tennis-green-pms-deploy/.ssh/authorized_keys` 只有一把 `restrict` deploy key，用户不能修改。
- 部署用户不在 `docker` 或 `sudo` 组，不能获得交互 shell、转发或任意 Docker 命令。
- `/opt/tennis-green-pms-release`、`/etc/tennis-green-pms`、`/var/lib/tennis-green-pms-release` 和入口文件由 root 控制。
- `tennis-green-pms-release-recovery.timer` 此时仍未启用。
...
```bash
rtk proxy scp /tmp/tennis-green-pms-onboarding/deploy.json \
  "$ADMIN_ALIAS:$REMOTE_STAGE/tennis-green-pms-deploy.json"
rtk proxy ssh "$ADMIN_ALIAS" \
  "sudo test ! -e /etc/tennis-green-pms/deploy.json && sudo install -m 0600 '$REMOTE_STAGE/tennis-green-pms-deploy.json' /etc/tennis-green-pms/deploy.json"
rtk proxy ssh "$ADMIN_ALIAS" \
  "sudo test ! -e /etc/tennis-green-pms/app.env && sudo install -m 0600 '$LEGACY_CHECKOUT/.env' /etc/tennis-green-pms/app.env"
rtk proxy ssh "$ADMIN_ALIAS" \
  'sudo test ! -e /etc/tennis-green-pms/cos-readonly.json && sudo install -m 0600 /dev/null /etc/tennis-green-pms/cos-readonly.json'
rtk proxy ssh -t "$ADMIN_ALIAS" 'sudoedit /etc/tennis-green-pms/cos-readonly.json'
```
...
Tennis 当前生产 Compose 只有 app 服务；AI 加密密钥等运行时变量通过 root-owned `app.env` 管理，不能把数据库密码、API Secret 或其他凭据写入仓库。
...
rtk proxy ssh "$ADMIN_ALIAS" 'sudo docker inspect --format "{{.Image}}" tennis-green-pms-app'
rtk proxy ssh "$ADMIN_ALIAS" 'curl --fail --silent http://127.0.0.1:4200/health'
```
...
rtk proxy node scripts/release/migration-baseline.mjs \
  --git OLD_REVISION > /tmp/tennis-green-pms-old-git-migrations.json
rtk proxy ssh "$ADMIN_ALIAS" \
  'sudo docker exec -i tennis-green-pms-app node --input-type=module - --image' \
  < scripts/release/migration-baseline.mjs \
  > /tmp/tennis-green-pms-old-image-migrations.json
rtk proxy diff -u \
  /tmp/tennis-green-pms-old-git-migrations.json \
  /tmp/tennis-green-pms-old-image-migrations.json
```
...
```bash
rtk proxy scp /tmp/tennis-green-pms-old-image-migrations.json \
  "$ADMIN_ALIAS:$REMOTE_STAGE/tennis-green-pms-migrations.json"
rtk proxy ssh "$ADMIN_ALIAS" \
  "sudo install -m 0600 '$REMOTE_STAGE/tennis-green-pms-migrations.json' /etc/tennis-green-pms/verified-migrations.json"
rtk proxy ssh "$ADMIN_ALIAS" \
  'sudo /usr/local/sbin/tennis-green-pms-deploy adopt OLD_TAG OLD_REVISION OLD_IMAGE_ID /etc/tennis-green-pms/verified-migrations.json'
rtk proxy ssh "$ADMIN_ALIAS" \
  'sudo systemctl enable --now tennis-green-pms-release-recovery.timer'
```
...
只创建 `production` Environment，允许 tag `v*` 和 branch `main`，不设置 reviewers/wait timer。Environment 的 branch/tag 限制不是身份授权的替代品；CAM policy 仍必须只允许 Tennis-Green-PMS 前缀。

首次真实发布前完成：

1. `rtk npm run test:release`、`rtk npm run release:check`、`rtk npm run typecheck`、`rtk npm test` 和 `rtk npm run build`。
2. GitHub Retention 在 `main` 上以 `dry_run=true` 运行，确认只列出 Tennis-Green-PMS 前缀且保护 current/previous。
...
```bash
rtk proxy ssh "$ADMIN_ALIAS" \
  'sudo systemctl status tennis-green-pms-release-recovery.timer --no-pager'
rtk proxy ssh "$ADMIN_ALIAS" \
  'sudo journalctl -u tennis-green-pms-release-recovery.service -n 50 --no-pager'
rtk proxy ssh "$ADMIN_ALIAS" \
  'sudo /usr/local/sbin/tennis-green-pms-deploy recover'
rtk proxy ssh "$ADMIN_ALIAS" \
  'sudo /usr/local/sbin/tennis-green-pms-deploy rollback-local'
```
