...
本地实现支持模拟付款、查单、回调和退款恢复。唯一已实现的适配器为显式本地 MOCK；没有真实微信 SDK、商户凭据、支付链接或二维码。WECHAT 商户配置只保存未来接入的身份和凭据引用，不能据此宣称已经接通。 F13 已新增产品共用的 API v3 签名/验签/通知解密安全层，尚未注册真实 provider 或发送 HTTP，边界见 [微信安全层](wechatpay-v3-security.md)。
