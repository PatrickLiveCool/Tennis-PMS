...
租户管理员在“设置 → 渠道身份”选择一个已有 PMS 客户或员工主体，填写经过渠道核验的 `externalSubjectId` 和核验依据，建立绑定。此处不按手机号/昵称自动注册、合并客户或授予余额权限。同一集成一个外部主体只允许一个有效绑定；变更身份必须撤销后重新人工绑定。旧消息和会话仍属于旧绑定，不能被新身份继承。
