...
F10 增加 `resourceType=exception_refund` 的 `refund.result`：仅对应未用于成交/充值入账的异常实收，金额为原路外部退款，`walletCents=0`。成功才关闭原异常；事件不包含退款原因或渠道交易号。客户只读本人结果；员工须有相应场馆权限及 `manage_members` 才可读这类事件。
